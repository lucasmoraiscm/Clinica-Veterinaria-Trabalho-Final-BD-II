...
BEGIN
	...
	IF EXISTS (
		...
		DESCONTO_BUSCADO := (
			SELECT DESCONTO_CONSULTA
			FROM VINCULO
			WHERE COD_VINCULO = COD_VINCULO_BUSCADO
		);
	ELSE
	...
	VALOR_PARCELA := ROUND((VALOR_TOTAL_CONS / QUANT_PARC_CONS), 2); -- VALOR DA PARCELA
	...
	VALUES (NOW(), NOW(), DIAGNOSTICO_CONS, COD_VET_BUSCADO, COD_ATEND_BUSCADO, COD_PAG_BUSCADO, COD_MEDICACAO_BUSCADO);
END;
$$ LANGUAGE PLPGSQL;
